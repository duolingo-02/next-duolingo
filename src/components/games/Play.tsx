...
const Play: React.FC = () => {
  ...
  const [currentStage, setCurrentStage] = useState(Number(stageId) || 1);

  const dispatch = useDispatch();

  const fetchLessons = async (stage: number) => {
    if (languageId && stage) {
      try {
        const response = await axios.get(`/api/lessons/${stage}/language/${languageId}`);
  ...
  const handleQuizComplete = async (quizPoints: number) => {
    ...
    if (currentLessonIndex < lessons.length - 1) {
      ...
      // All lessons completed, increment stageId and fetch next lesson
      const nextStage = currentStage + 1;
      try {
        await fetchLessons(nextStage);
